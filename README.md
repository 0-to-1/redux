...
アプリのすべての状態は、一つの *Store* 内にある一つのオブジェクトツリーで保持されます。状態ツリーを変更する唯一の方法は、*Action* を送ることです。Actionは、何が起きたのかを説明するオブジェクトです。どのようにActionが状態ツリーを転換するか明示するために、純粋な（副作用を起こさない）*Reducer* を書きます。
